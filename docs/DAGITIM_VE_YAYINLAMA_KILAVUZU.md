...
1. 💻 **`RADPYS_V3` (Geliştirme Reposu - Private):** Tüm Python kaynak kodları, veritabanı şemaları, iş mantığı ve yerel birim testlerinin bulunduğu ana geliştirme ortamıdır.
...
3. 🌐 **`RADPYS_V3_WEB` (Web Portal & Yayınlama - Cloudflare):** Cloudflare Pages ve Cloudflare R2 depolama servisi üzerinde çalışan, kullanıcılara üyelik/giriş gerekmeden doğrudan indirme sunan web portalıdır.
...
```mermaid
graph TD
    A["1. RADPYS_V3 (Geliştirme)<br/>Kodlar yazılır & test edilir"] -->|Temiz Kod Senkronizasyonu| B["2. RADPYS_Release (Test)<br/>git push main -> CI testleri doğrular"]
    B -->|Sürüm Etiketi| C["3. Tag Push (git tag v3.8.1)<br/>Derleme & Paketleme başlar"]
    ...
    E -->|Doğrudan İndirme| F["4. RADPYS_V3_WEB (Web Portal)<br/>Kullanıcılar tek tıkla doğrudan indirir"]
```
...
git add .
git commit -m "chore: v3.8.1 sürüm kodları aktarıldı"
git push origin main
```
...
# 2. Yeni versiyon etiketini oluşturun (Örn: v3.8.1)
git tag v3.8.1
...
> 3. `Inno Setup` ile `RADPYS_Setup_v3.8.1.exe` kurulum paketi hazırlanır.
...
### 🔹 4. AŞAMA: Web Sitenizdeki (`RADPYS_V3_WEB`) İndirme Butonu
...
1. **%100 Kod Gizliliği:** `RADPYS_V3` ve `RADPYS_Release` depoları **Private (Gizli)** ayarlanmıştır. Dışarıdan hiç kimse Python kaynak kodlarınızı göremez.
