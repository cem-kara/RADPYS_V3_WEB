...
18. Sık Karşılaşılan Durumlar ve İpuçları
19. Update Log
...
- PySide6 tabanlı masaüstü arayüz: RADPYS V3, bilgisayarınıza kurularak çalıştırılan bir masaüstü uygulamasıdır; internet tarayıcısı gerektirmez.

- SQLite veritabanı: Tüm veriler, uygulamayla birlikte çalışan yerel bir SQLite veritabanı dosyasında saklanır. Bu yapı, Veritabanı Modülü altındaki yedekleme ve geri yükleme işlemlerinin temelini oluşturur (bkz. Bölüm 12).

- Python 3.14+ üzerinde geliştirilmiştir.
...
"Şifremi Unuttum" bağlantısına tıkladığınızda, sistem şifrenizi sıfırlamak için açılan pencerede email,kullanıcı adı ve yeni şifrenizi girmeniz gerekmektedir. Bu bilgilerle sistem şifrenizi sıfırlayabilirsiniz. Eğer bu ekran gelmez ise, sistem yöneticinizle iletişime geçmeniz gerekmektedir.
...
### 2.5 Gösterge Paneli Merkezi (Dashboard Hub)
...
- **Genel Bakış (Overview):** Sistem genel durumunu, aktif çalışan sayısını, bekleyen izin taleplerini, yaklaşan/geciken periyodik sağlık muayenelerini gösterir. Sağ paneldeki "Hızlı Erişim & İşlemler" alanı üzerinden yetkili olduğunuz modül ekranlarına hızlıca geçiş yapabilirsiniz.
- **Nöbet Analizi:** Birimlerin aylık nöbet saati yüklerini ve nöbet/fazla mesai trend grafiklerini içerir. Onay bekleyen nöbet devir talepleri bu panelden satır seçilerek "Seçileni Onayla" veya "Seçileni Reddet" butonlarıyla kolayca yönetilebilir.
...
- **Olay Analizi:** Radyasyon güvenliği ve olay bildirim kategorilerinin dağılımını ve aylık bildirim trend grafiklerini listeler.
...
2. Yeni bir rol tanımlamak için "Yeni Ekle" butonuna tıklayın. Açılan formda rol adı, rolün aktiflik durumu ve **Rol Kapsamı (Scope)** bilgisini girin.
...
**Rol Kapsamı (Satır Düzeyinde Güvenlik):**

Rol oluştururken veya güncellerken seçebileceğiniz üç farklı kapsam seçeneği bulunmaktadır:

- **own (Sadece Kendisi):** Bu role sahip kullanıcılar, Personel, İzin, Sağlık Muayene, Dozimetre ve Fiili Hizmet sayfalarında yalnızca kendi personel kayıtlarını görebilir ve yönetebilirler (Örn: `kullanici`, `izleyici` rolleri).
- **department (Kendi Departmanı):** Kullanıcılar sadece bağlı oldukları alt departmandaki çalışanların kayıtlarını görebilir ve yönetebilirler. Sistem, yetkilendirmeyi metin eşleşmesi yerine veritabanı ilişkisel düzeyiyle (**alt_departman_id**) güvenli ve hızlı bir şekilde sorgular.
- **all (Tümü):** Kullanıcılar sistemdeki tüm departman ve personellerin verilerine tam yetkiyle erişebilirler (Örn: `admin`, `yonetici` rolleri).
...
![alt text](images/modül_yetki.png)
...
- **Modül Yetkileri Sekmesi:** Her bir rol için modüller bazında standart Okuma, Yazma, Güncelleme ve Silme (CRUD) haklarının atandığı tablodur.
  - **Kapsam (Yetki Alanı) Seçimi:** Her modül satırında, checkbox yetkilerinin yanı sıra **Miras**, **Kendisi**, **Departman** ve **Tümü** şeklinde yan yana konumlandırılmış **radyo buton grubu** yer alır.
  - **Dinamik Açıklama Sütunu:** Seçilen radyo butonuna göre modülün sağ tarafındaki **Açıklama** alanında ilgili kapsam kuralının gerçek Türkçe karşılığı anlık olarak gösterilir (Örn: *Sadece Kendisi (kendi kayıtları)* veya *Miras Al (Rolün genel ayarı)*).
  - **Akıllı Toplu İşlemler:**
    - **Tümünü Temizle** butonu tüm modüllerdeki yetki checkbox'larını kaldırırken kapsam sınırlarını da **Miras** (varsayılan) durumuna geri döndürür.
    - **Şablon Uygula** ile hazır bir yetki grubu uygulandığında (Örn: *Tam Yetki*, *Sadece Okuma*), matrisin kapsamları temiz bir başlangıç için otomatik olarak **Miras** moduna resetlenir.
...
**Rol Yetki Karşılaştırma (Popup Detay):**

Üst paneldeki **"Karşılaştır"** combobox'ından hedef bir rol seçilip **"Karşılaştır"** butonuna basıldığında, ana ekranı bozmadan yan yana detay sunan modern bir **Rol Karşılaştırma Detay** pop-up penceresi açılır. Bu pencerede:
...
- Kapsam değeri belirtilmeyen (None) modüller matriste net bir şekilde **"Miras Al (Rolün Genel Kapsamını Alır)"** olarak etiketlenerek gösterilir.

![alt text](images/rol_yetki.png)

**İşlem Adımları:**

1. Rol Yetkileri ekranına Kullanıcı Modülü > Rol Yetkileri yolundan ulaşın.
2. Üst kısımdaki combobox'tan yetkilerini düzenlemek istediğiniz rolü seçin.
3. İlgili yetki sekmelerindeki (Modül Yetkileri veya İnce Ayarlı Yetkiler) onay kutularını ve kapsam radyo butonlarını düzenleyin.
...
- `nobet/devir_talebi`: Nöbet çizelgesinde "Nöbet Devir Talebi" butonunun aktif kalmasını sağlar.
- `nobet/plan_onaylama`: Nöbet plan detay ekranında planın durumunun "Taslak"tan "Onaylandı" veya "Yayında" durumuna getirilmesine izin verir.
- `personel/toplu_aktarim`: Personel listesinde Excel'den toplu içe aktarım eylemini yönetir.
- `dozimetre/aksiyon_baslatma`: Dozimetre erken uyarı listesinde sağ tık menüsünden yeni aksiyon başlatma haklarını denetler.

> **Not:** Yeni bir kurulumda rol-yetki eşleştirmeleri sistemle birlikte önceden tanımlanmış (seed edilmiş) olarak gelir; bu nedenle kurulum sonrasında temel rollerin yetkilerini ayrıca tanımlamanıza genellikle gerek kalmaz.
...
![alt text](images/sifre_edit.png)
...
![alt text](images/personel_ekle.png)
...
4. Cinsiyet alanında "E" (Erkek) veya "K" (Kadın) seçimini yapın; seçiminize göre sistem varsayılan bir avatar görseli atar.
5. İsterseniz "Profil Resmi Yükle" alanından personelin fotoğrafını yükleyin.
6. Formu kaydedin.

> **Not:** Yüklenen profil fotoğrafı sistemde `data/uploads/personel/{TC_NO}/{TC_NO}_profil.{uzantı}` klasör yapısında saklanır; bu yapı sayesinde her personelin dosyaları kendi TC kimlik numarasına ait klasörde tutulur.
...
> **Not:** Yüklenen dosyaların orijinal adları sadece veritabanında kayıt edilir; arayüzde dosyalar sistem tarafından standart bir isimlendirme ile (örn. `data/uploads/personel/TC_NO/egitim/TC_NO_belgetipi_egitimid.uzantı`) gösterilir.
...
3. Şablondaki sütunları (TC kimlik no, ad-soyad, departman, unvan, iletişim bilgileri vb.) eksiksiz doldurun.
...
- Yüklenen dosyaların orijinal adları yalnızca veritabanında saklanır; arayüzde dosyalar standart bir adlandırma kuralıyla görüntülenir (örn. `data/uploads/personel/TC_NO/TC_NO_profil.uzantı`).
...
- İzinle ilgili yüklenen belgeler `data/upload/personel/TC_NO/izin/TC_NO_belgetipi_izinid.uzantısı` klasör yapısında saklanır.
...
### 6.3 Görev Dağılımı ve Hesaplama UX Özellikleri

Geliştirilen modern arayüz kapsamında aşağıdaki kolaylıklar ve kontroller sağlanmıştır:

- **Hızlı Dönem Navigasyonu:** Dönem filtrelerinin yanında yer alan (◀ ▶) yönlendirme butonları yardımıyla aylar arasında hızlıca geçiş yapabilirsiniz. Dönem geçişi yapıldığında diğer tüm sekmelerin (Hesaplama, Puantaj vb.) dönem filtreleri otomatik olarak senkronize edilir.
...
- **Proaktif Taslak Uyarısı:** Dönemde onay bekleyen taslak kayıtlar olduğunda, sayfa üstünde sarı renkli uyarı bandı belirecektir: `⚠️ Bu dönemde onaylanmamış taslak kayıtlar bulunuyor. Hesaplama yapmadan önce lütfen toplu onaylama yapın.`
- **Toplu Onaylama Yeteneği:** "Toplu Onayla" (`btnBulkApprove`) butonu sayesinde dönemdeki tüm taslak ve kaydedilmemiş default atamaları tek tıkla onaylayabilirsiniz.
...
**Geçmiş Arama:** Muayene Listesindeki "Geçmiş" (`gecmisButton`) butonunu veya tablodaki satıra sağ tıklayarak açılan "Personel Muayene Geçmişi" seçeneğini kullanarak, seçili personelin geçmişe dönük tüm sağlık muayenelerini kronolojik bir liste halinde görüntüleyebilirsiniz.
...
- **Zorunlu Alanlar ve Doğrulama:** Sağlık muayenesinin kaydedilebilmesi için en az bir branş sonucunun seçilmesi zorunludur. Tüm branşlar "Seçiniz..." bırakılarak kayıt yapılamaz. Seçilmeyen ("Seçiniz...") branşlar veritabanına string olarak değil, `None` (NULL) olarak güvenli şekilde kaydedilir.
- **Kaydetme Onay Adımı:** Personel detay sayfasındaki sağlık sekmesinden muayene kaydederken veya güncellerken kazara veri kayıplarını ve hatalı kayıtları önlemek amacıyla sistem size bir onay penceresi (`ask_confirm`) sunar.
- **Çapraz Platform Belge Desteği:** Muayene belgelerini görüntülemek için "Aç" butonuna bastığınızda, sistem işletim sistemi fark etmeksizin (Windows, macOS, Linux) dosyayı varsayılan uygulamasıyla güvenli bir şekilde (`QDesktopServices.openUrl` kullanarak) açar.
...
- "Temizle" butonuna bastığınızda belge hem diskten hem de veritabanından silinir; bu işlem geri alınamaz, dikkatli kullanın.
...
3. Dosyayı yükleyin; sistem dosyayı okuyarak ölçüm sonuçlarını ilgili personel kayıtlarıyla eşleştirip veritabanına işler.
...
Bu modül, sistemin genelinde kullanılan temel referans verilerinin (departman, unvan, izin türü, tatil günleri vb.) tanımlandığı bölümdür. Buradaki tanımlar, diğer modüllerdeki seçim listelerinde (dropdown) kullanılır.
...
Rapor Şablonları ekranı, kurumların resmi evrak ve yazışma çıktılarını kendi taslaklarına (.docx formatında Word dosyaları) göre düzenleyebilmesini sağlar.

- **Jinja2 Şablon Desteği:** Şablonlar içerisindeki metin yer tutucuları Jinja2 formatında (`{{BASLIK_1}}`, `{{BASLIK_2}}` vb.) güvenli şekilde tanımlanabilir.
- **Güvenli Dönüşüm (docxtpl):** `docxtpl` kütüphanesi sayesinde Word dosyaları güncellenirken XML yapısı korunur, split-run hataları ve özel karakterlerden kaynaklanan dosya bozulmaları engellenir.
...
Raporlar Modülü, sistem üzerinde gerçekleştirilen işlemlerin (kayıt ekleme, düzenleme, silme, giriş/çıkış vb.) log (işlem kaydı) bilgilerini görüntülemenizi sağlar.

1. Sol menüden Raporlar Modülü ekranına gidin.
2. Tarih aralığı, kullanıcı veya modül bazında filtreleme yaparak ilgili log kayıtlarını görüntüleyin.
3. Gerekirse kayıtları listeleyip inceleyin.

### 10.1 Birleşik Dışa Aktarma Sistemi (Unified Export)
...
- **Desteklenen Formatlar:** Excel (`.xlsx`), Word (`.docx`), CSV (`.csv`) ve PDF (`.pdf`) formatları desteklenmektedir.
- **Rapor Tasarımı ve Stiller:** Excel çıktılarında kurumsal başlıklar, logolar, otomatik sütun genişlikleri ve kalın header satırları kullanılır. PDF çıktılarında ise Türkçe karakter setlerine uygun, sayfa numaralandırmalı ve otomatik yatay/dikey sayfa yönlendirmeli raporlar üretilir.
- **Dinamik Şablon ve Konum Eşleme (Excel):** Rapor şablonlarında (`data/templates/` altındaki `.xlsx` dosyalarında) sütunların sırasını değiştirebilir, bazı sütunları silebilir veya yeni başlıklar ekleyebilirsiniz. Akıllı konum eşleme algoritması, başlık satırını (`{{VERI}}` satırının hemen üstünü) okuyup normalize ederek veriyi her zaman doğru başlığın altına yazar. Böylece şablonları istediğiniz gibi özelleştirebilirsiniz.
- **Örnek Şablonlar ve Word (.docx) Entegrasyonu:** Tüm rapor tipleri için kurumsal marka ve logo yer tutucularını (`{{BASLIK_1}}`, `{{LOGO_1}}`, `{{RAPOR_BASLIGI}}` vb.) barındıran temiz Word şablonları (`.docx`) oluşturulmuştur. Dışa aktarma yapıldığında bu şablonlar kullanılarak profesyonel belgeler üretilir.
- **Kullanım:** İlgili liste ekranlarındaki "Dışa Aktar" butonuna tıkladığınızda açılan dosya kaydetme diyaloğundan istediğiniz formatı seçerek dosyayı kaydedebilirsiniz.
...
1. **Sağlık Muayene Raporu:** Personelin temel kimlik ve departman bilgilerinin yanında branş bazlı Göz, Dahiliye, Dermatoloji muayene tarihlerini ve onay/imza durumlarını (toplam 22 sütun) içerir.
2. **Dozimetre Ölçüm Raporu:** Personelin derin/yüzeysel doz ölçümleri, kümülatif dozları, limit aşımı ve laboratuvar rapor no/tarih verilerini (toplam 21 sütun) listeler.
3. **İzin Bakiye Raporu:** Yıllık ve Şua izinleri için hakedilen, kullanılan, devredilen, dondurulan ve kalan gün kırılımlarını (toplam 14 sütun) listeler.
4. **Eğitim Durum Raporu:** Personelin eğitim türü, mezuniyet yılı ve onay durumunu listeler.
5. **Kimlik ve İletişim Bilgileri Raporu (Yeni):** Personelin doğum tarihi/yeri, cinsiyeti, medeni hali, anne/baba adı, telefon/e-posta adresleri, il/ilçe bilgileri, işten çıkış tarihi/nedeni, nöbet ve fazla mesai durumlarını listeleyen kapsamlı bir kişisel veri dökümüdür.

### 10.3 Opsiyonel ve Dinamik Sütun Sağlayıcılar (FieldProviders)

Şablon Ayarları sayfasından etkinleştirilebilen opsiyonel alanların yanında, veritabanından dinamik birleştirilmiş veri çeken sağlayıcılar da sisteme entegre edilmiştir:

- **Eğitim Dökümü (`egitim_dokumu`):** Personelin onaylı eğitim geçmişini listeler.
- **İzin Bakiyesi (`izin_bakiyesi`):** Yıllık ve Şua izin özetini verir.
- **Son Dozimetre Ölçümü (`dozimetre_son_olcum`):** En son kaydedilen dozimetre sonuçlarını özetler.
- **Dozimetre Atama Durumu (`dozimetre_atama_durumu`):** Aktif atamaları gösterir.
- **Dozimetre Atama Detayı (`dozimetre_atama_detay`):** Dozimetre cihazlarının seri no, cihaz türü ve aktif atama tarihlerini listeler.
- **Son Sağlık Muayenesi (`saglik_muayene_son_tarih`):** En son muayene tarihini verir.
- **Aktif Çalışma Kısıtları (`calisma_kisitlari`):** Personelin aktif çalışma kısıtlarını gösterir.
- **Çalışma Kısıtı Detayı (`calisma_kisiti_detay`):** Kısıt tipi, gerekçe, günlük/aylık saat azaltım miktarları ve geçerlilik tarihlerini tek bir hücrede birleştirir.
- **Aylık Nöbet Özeti (`nobet_aylik_ozet`):** Ay içerisindeki toplam nöbet ve çalışma saatini verir.
- **Kayıtlı Belgeler (`belge_listesi`):** Personel özlük dosyasına yüklenmiş tüm belgelerin türlerini ve veriliş tarihlerini listeler.
- **Son İzin Kaydı (`izin_gecmisi_son`):** Personelin talep ettiği en son iznin türü, tarihleri ve onay durumunu listeler.
...
5. **Hafta Sonu / Bayram Maksimum Nöbeti:** Personelin bir ay içinde alabileceği maksimum hafta sonu ve resmi tatil nöbet sayıları. Bu alandaki *Ayda max hafta sonu nöbeti* (`weekendMaxInput`) kısıtı tamamen global bir limit olup, herhangi bir birime özel ezme (override) içermeksizin tüm kurum genelinde uygulanır.
...
- **Yumuşak Kısıtlarda Manuel Override Açık:** Aktif edildiğinde, yöneticiler sistem tarafından oluşturulan çizelgedeki esnek kuralları manuel olarak düzenleyebilir ve kuralları esnetebilir.
...
- **Yayindeki Plan Salt Okunur:** Onaylanıp yayınlanmış planlar üzerinde doğrudan ekleme, düzenleme veya silme (CRUD) işlemlerini engeller. Plan salt okunur kalır; sadece devir işlemleri yapılabilir.
...
- **Devredilen Nöbetlerin Düzenlenmesini Engelle:** Bu ayar etkinleştirildiğinde, durumu 'Devir' (transfer işlemi tamamlanmış) olan nöbetlerin manuel olarak düzenlenmesi veya silinmesi engellenir. Ayar kapatıldığında, yetkili yöneticiler devredilmiş nöbetleri de serbestçe düzenleyebilirler.
...
- **Varsayılan Kısıt Türleri:**
  - *Nöbet Sonrası Dinlenme Saati* (`nobet_sonrasi_dinlenme`): Nöbeti biten bir personelin bir sonraki nöbetine kadar dinlenmesi gereken zorunlu asgari dinlenme süresi (Zorunlu kısıttır, varsayılan `24` saat).
  - *Maksimum Art Arda Nöbet Günü* (`art_arda_max`): Personelin üst üste en fazla kaç gün nöbetçi olabileceği (varsayılan `2` gün).
  - *Gece Nöbetleri Arası Minimum Boşluk* (`gece_nobeti_min_bosluk`): İki gece nöbeti arasında geçmesi gereken minimum dinlenme süresi (varsayılan `48` saat).
  - *Hafta Sonu Nöbet Dengesi / Limiti* (`hafta_sonu_dagilim`): Personelin bir ay içerisinde alabileceği maksimum hafta sonu nöbet limiti (varsayılan `4` nöbet).
  - *Bayram Nöbet Dengesi / Limiti* (`bayram_dagilim`): Personelin bayram günlerinde alabileceği maksimum nöbet limiti (varsayılan `2` tatil nöbeti).

- **Kısıt Çözümleme Öncelik Hiyerarşisi:**
  Nöbet planlamasında ve ayarlar ekranında kısıtlar şu 3-seviyeli hiyerarşiye göre önceliklendirilir:
  **Birim Kuralları > Vardiya Kısıtları (Birim & Sınıf Bazlı) > Temel Ayarlar**
  ...
  3. **Global/Kurumsal Kural:** Eğer özel bir kural yoksa, Tüm Birimler / Tüm Hizmet Sınıfları (`NULL` değerler) için tanımlanmış global seed/başlangıç değerleri devreye girer.
...
2. **Fazla Mesai Muafiyeti (Mesai Dışı - fm_off):** Nöbet istekleri ekranından bu ekrana taşınmıştır. Aktifleştirildiğinde, seçili personelin nöbet dışındaki normal çalışma saatlerini aşan fazla mesailerden (`fm_off` parametresi ile) muaf tutulmasını sağlar.
...
Birimlerin çalışma modellerinin, aktiflik takvimlerinin ve günlük personel (slot) ihtiyaçlarının belirlendiği alandır. Mükerrer kısıt girişlerinin önlenmesi amacıyla ardışık gün limiti, dinlenme süresi gibi sayısal sınırlar bu sekmeden tamamen kaldırılmış ve **Vardiya Kısıtları (Birim & Sınıf Bazlı)** sekmesine devredilmiştir.

**A. Birim Genel Kuralları (Takvim Kuralları):**

Birim Kuralları sekmesinde, seçili birimin takvim ve çalışma izinleri 2x2 grid yapısındaki onay kutularıyla yönetilir:
- **Hafta Sonu Çalışması Aktif (`weekendWorkCheck`):** İlgili birimde hafta sonu günlerinde nöbet yazılıp yazılamayacağını belirler. Örneğin 08-15 mesai yapan polikliniklerde bu kutucuk kapatılarak hafta sonu nöbet yazımı engellenir; acil servis gibi 7/24 çalışan birimlerde ise aktif tutulur.
- **Bayram Çalışması Aktif (`holidayWorkCheck`):** Birimde resmi tatillerde ve bayramlarda nöbet tutulup tutulmayacağını belirler.
- **24 Saat Tutulabilir (`combine24hCheck`):** Birimdeki 12 saatlik vardiyaların (Gündüz ve Gece) birbirini takip edecek şekilde tek bir 24 saatlik nöbet bloğu olarak atanıp atanamayacağını belirler.
- **Çapraz Görevlendirme (`slotSeyyarCheck`):** Personelin kendi ana birimi dışındaki diğer nöbet birimlerine de atanabilmesine izin verir.
- **Birim Kurallarını Kaydet Butonu:** Yukarıda listelenen takvim kuralları (*Hafta Sonu Çalışması Aktif*, *Bayram Çalışması Aktif*, *24 Saat Tutulabilir*, *Çapraz Görevlendirme*) genel "Temel Ayarlar" kaydetme mantığından ayrıştırılmıştır. Bu kurallardaki değişikliklerin kaydedilebilmesi için ilgili alanların hemen altındaki **"Birim Kurallarını Kaydet"** butonuna tıklanmalıdır. Seçili birim `"Genel (Birim Bağımsız)"` ise bu ayarlar tüm birimler için varsayılan (global) kural olarak, belirli bir birim seçili ise o birime özel kural (override) olarak kaydedilir. (Not: Hafta sonu nöbet sayı limiti global bir ayar olduğu için birim kurallarından tamamen çıkarılmıştır).

**B. Birim Çalışma Şeması (Nöbet Türleri ve Slot Sayıları):**

- **Arama/Filtreleme:** Nöbet ayarlarındaki Birim Kuralları sekmesinde yer alan **Birim Filtresi (Combobox)** kullanılarak, sadece seçilen birime ait nöbet kuralları ve türleri anlık olarak tabloda filtrelenebilir. "Tümü" seçeneği seçilerek tüm birimlerin kurallarına tek ekrandan ulaşılabilir.
...
- **Gün Kısıtı (Vardiya Gün Kısıtlaması):** Her bir nöbet türünün (vardiyasının) takvimdeki hangi gün tiplerinde aktif olacağını belirler. Aday filtreleme motoru bu kuralı sert kısıt olarak uygular. Seçenekler:
  - *Her Gün (`her_gun`):* Nöbet türü hafta içi, hafta sonu, resmi tatil fark etmeksizin her güne yazılabilir. (Örn: Acil Servis Nöbeti).
  - *Sadece Hafta İçi (`hafta_ici`):* Nöbet türü yalnızca hafta içi günlerde çalıştırılır. Hafta sonlarında veya resmi/dini bayram tatillerinde planlanmaz. (Örn: Poliklinik/Rutin Mesai Nöbeti).
  - *Sadece Hafta Sonu ve Tatil (`hafta_sonu_tatil`):* Nöbet türü hafta içine yazılmaz, sadece Cumartesi-Pazar günleri ve resmi tatillerde planlanır. (Örn: Hafta sonu icap nöbeti).
...
- **Günlük Slot İhtiyaçları:** Hangi nöbet türünde, haftanın hangi günlerinde kaç personelin nöbetçi olması gerektiği (slot sayısı) buradan yapılandırılır. Otomatik planlayıcı bu slotları dolduracak şekilde çalışır.
...
- **Talep Türleri:**
  - *Nöbet Yazılmasın (Mazeret):* Personelin o tarihlerde nöbet tutamayacağını belirtir (örn. kişisel mazeret). Nöbet planlayıcı strict aşamada bu mazereti kesinlikle korur. Eğer kadro yetersiz kalırsa ve relaxed aşamada esnetme gerekirse; düşük öncelikli (1-3) mazeretler esnetilebilirken, yüksek öncelikli (4-5: Çok Yüksek ve Kritik) mazeretler asla ihlal edilmez.
...
- **Yeni Plan Ekleme:**
  ...
  2. Açılan diyalog penceresinde Plan Yılı ve Plan Ayı seçin.
...
- **Plan Silme / Arşivleme:**
  ...
  *Önemli:* "Yayında" durumundaki planlar doğrudan silinemez veya normal kullanıcılar tarafından durumları değiştirilemez. Sadece yetkili **Admin** ve **Yönetici** kullanıcılar bu planları "Arşiv" statüsüne taşıyabilir ya da "Taslak" moduna geri çekebilir.
...
- **Otomatik Nöbet Oluşturma:**
  ...
  3. İşlem sırasında bir ilerleme çubuğu (progress bar) gösterilir. Kullanıcı dilerse işlemi yarıda iptal edebilir.
  4. Hesaplama bittiğinde oluşturulan çizelge taslak olarak ekrana yansıtılır. "Kaydet" butonu ile çizelge veritabanına kaydedilir.

  > [!NOTE]
  > **Otomatik Nöbet Oluşturma Sırasında Taslak Yedeği ve Geri Yükleme:**
  > Mevcut taslak/planlı bir nöbet listesi üzerinde tekrar "Otomatik Plan Oluştur" butonuna tıklandığında, eski taslak kayıtlar kalıcı olarak silinmeden önce sistem tarafından otomatik olarak `data/backups/nobet/plan_{plan_id}_{timestamp}.json` yoluna JSON formatında yedeklenir.
  > 
  > Yanlışlıkla silinen veya üzerine yazılan taslak çalışmalarınızı geri kurtarmak için çizelge detay ekranındaki **"Yedekten Taslak Yükle"** butonuna tıklayıp ilgili `.json` dosyasını seçerek çizelgeyi o yedek anına geri döndürebilirsiniz.
  > 
  > Disk kirliliğini önlemek amacıyla, nöbet planı **Yayına alındığında**, **Arşivlendiğinde** veya **Silindiğinde**, o plana ait tüm geçmiş taslak yedek dosyaları sistem tarafından diskten **otomatik olarak temizlenir**.
...
- **Manuel Düzenleme (Çizelge Hücre İşlemleri):**
  ...
  - Yönetici, kısıt ihlali uyarısına rağmen (opsiyonlarda override açık ise) onaylayarak manuel atamayı gerçekleştirebilir.
...
4. *Önemli Kural:* "Yayında" durumuna getirilen nöbet planları üzerinde yöneticiler dahil doğrudan CRUD (ekleme, silme, personel değiştirme) işlemleri yapamaz. Çizelge salt okunur hale gelir. Bu aşamadan sonra sadece **Nöbet Devir** süreçleri işletilebilir.
5. Yetkili kullanıcılar gerekirse planı tekrar düzenlemek için **"Taslağa Geri Çek"** (`revertToDraftButton`) seçeneğini kullanabilir. Bu işlem yetki sınırlandırmasına tabidir ve normal personel kullanıcıları tarafından gerçekleştirilemez.
...
- **Nöbet Devir Talebi Oluşturma:**
  ...
  1. Çizelge detay veya İnceleme ekranında devredilmek istenen nöbet hücresine tıklayın ve **"Devir Talebi Oluştur"** (`transferButton`) butonuna basın. Bu buton, sadece nöbetin asıl sahibi olan personel veya sistemdeki yetkili **Yönetici** ve **Admin** rolleri için aktiftir. Başkasına ait nöbetler için devir talebi oluşturulamaz.
...
- **Taleplerin Onaylanması ve Çizelgenin Güncellenmesi:**
  ...
  - *Yönetici/Admin Bypass:* Acil durumlarda, sistem yöneticisi (Admin) tarafından yapılan onaylama işlemlerinde çok adımlı hiyerarşik onay akışları (personel -> birim sorumlusu -> yönetici) baypas edilerek devir işlemi tek adımda doğrudan tamamlanabilir.
...
### 11.6 Nöbet Fazla Mesai Borç / Alacak Devri (Carryover)
...
- **Borç ve Alacak Durumunun Belirlenmesi:**
  ...
  - *Net Fark (Saat) > 0 ise:* Kurum personele borçludur, personel alacaklı durumdadır (fazla mesai yapmış).
  - *Net Fark (Saat) < 0 ise:* Personel kuruma borçludur, kurum alacaklı durumdadır (eksik çalışma yapmış).

- **İşlem ve Devir Kararları:**

  Yöneticiler, tablonun son sütunundaki seçim kutusunu (ComboBox) kullanarak her personel için borç/alacak kararını belirleyebilir:

  - *Beklemede:* Karar henüz verilmemiş, işlem bekleniyor.
  - *FMOdendi:* Personelin fazla mesai alacağı nakdi olarak ödenmiş sayılır.
  - *İzinKullanıldı:* Alacaklı saatler karşılığında personele izin kullandırılmıştır.
  - *SonrakiAydanDusuldu / SonrakiAyaDevredildi:* Borç veya alacak saati bir sonraki aya devredilir.

    - *Etki:* Devredilen borç saatleri personelin sonraki aydaki hedef süresine eklenir (daha fazla çalışması gerekir). Alacak saatleri ise sonraki aydaki hedef süresinden düşülür veya sonraki ayın nöbet planlama öncelik skorunu yükseltir.

- **Değişikliklerin Kaydedilmesi:**

  Kararlar belirlendikten sonra üstteki **"Tüm Kararları Kaydet"** (`btnSaveAll`) butonu ile veritabanına kaydedilir. Kayıt sonrasında devreden farklar sonraki ayın nöbet atamalarında otomatik olarak devreye girer.

---

---

## 12. Radyasyon Güvenliği ve Kalite Olay Bildirim / DÖF Modülü
...
### 14.1 Yeni Olay Bildirimi (Wizard/Stepper Sihirbazı)
...
- **Adım 2: Sınıflandırma:** Olay kategorisi (Radyasyon İhlali Çalışan Odaklı, Hasta Odaklı, MR Güvenliği vb.) seçilir. Seçilen kategoriye göre veritabanındaki hiyerarşik yapıya uygun alt detay seçenekleri dinamik check-box'lar halinde yüklenir. Kök neden checklistinden olası sebepler işaretlenir.
- **Adım 3: Açıklamalar:** Detaylı olay tanımı, yapılan acil müdahale ve DÖF önerisi girilir. Form "Olayı Bildir" ile kaydedildiğinde veritabanı trigger'ı otomatik olarak benzersiz bir takip numarası üretir (Örn: `OB-2026-00001`).

### 14.2 Olay Bildirimleri ve DÖF Yönetimi (Yönetici Paneli)

Yöneticiler sol menüdeki **YÖNETİM > Olay & DÖF Yönetimi** ekranı üzerinden bildirilen olayları takip ederler:
...
- **Tarihçe (Audit Log):** Olay üzerinde yapılan tüm durum ve alan güncellemeleri otomatik olarak eski/yeni değerleriyle tarihçeye loglanır.
...
RADPYS V3, kurumsal veri güvenliğini, veri doğruluğunu ve denetim izlenebilirliğini en üst düzeyde tutmak amacıyla **Evrensel Onay ve Veri Değişikliği Denetim Sistemi** ile donatılmıştır. Bu sistem; rol tanımlarında `onay_gerektirir` ayarı `1` olan (örneğin Kullanıcı, İzleyici veya özel tanımlanmış veri giriş personelleri) rollerin yaptığı ekleme, düzenleme veya silme işlemlerini doğrudan veritabanına yazmak yerine, bir yöneticinin onayına sunulmak üzere geçici bir onay kuyruğuna (`degisiklik_talepleri`) yönlendirir.

Sistemde iki farklı onay yaklaşımı bulunmaktadır:

1. **Onay Kuyruğu Destekleyen Modüller:** Bu modüllerde yapılan işlemler reddedilebilir veya onaylanabilir. Onaylanana kadar veriler asıl veritabanı tablolarına işlenmez.
2. **Onay Kuyruğu Desteği Olmayan Modüller (Sert Yetki Engeli):** Bu modüller yasal uyum ve hassas izleme verileri içerdiğinden onay kuyruğu üzerinden onaylanmaya uygun değildir. Bu nedenle onaya tabi (`onay_gerektirir = 1`) rollerin bu modüllerde işlem yapması doğrudan sistem tarafından engellenir.

Yöneticiler, bekleyen tüm onay taleplerini ana ekranda (Dashboard) yer alan **"Onay Bekleyen Görevler"** hızlı erişim butonuyla veya sistem menüsünden açılan merkezi onay panelinden yönetebilirler. Onaylama veya reddetme işlemlerinde işlemi gerçekleştiren yöneticinin personel kimliği (`actor_personel_id`) ve rol bilgisi (`actor_role`) denetim izleri için eksiksiz olarak doğrulanır ve veritabanına kaydedilir.

### 15.1 Kategori Sekmeleri ve Sayı Rozetleri (Badges)
...
4. **Nöbet Planları:** Birim yöneticileri tarafından hazırlanmış ("Birim Onaylı" durumundaki) ve yayınlanmadan önce admin onayı bekleyen nöbet planları.
5. **Veri Değişiklikleri:** Onay gerektiren kullanıcılar tarafından aşağıdaki modüllerde yapılan ekleme, düzenleme ve silme talepleri:
   - **Personel Özlük Bilgileri** (`personeller` tablosu)
   - **Sağlık Muayeneleri** (`personel_saglik_muayene` tablosu)
   - **Eğitim Türleri / Tanımlamaları** (`egitim_turleri` tablosu)
   - **Kalite Dokümanları Portalı** (`kalite_dokumanlari` tablosu)
   - **Hizmet İçi Eğitim Tanımları** (`hizmet_ici_egitimler` tablosu)

### 15.2 Karşılaştırmalı Değişiklik İnceleme (Diff Dialog)
...
- **Detaylı Alan Eşleştirmeleri (Field Map):** Eğitim ve Kalite modüllerinin de onay sistemine entegre edilmesiyle, karşılaştırma ekranındaki ham veritabanı kolon isimleri kullanıcı dostu Türkçe etiketlere dönüştürülmüştür:
  - `egitim_adi` $\rightarrow$ **Eğitim Adı**
  - `seviye` $\rightarrow$ **Eğitim Seviyesi**
  - `sure_dakika` $\rightarrow$ **Süre (Dakika)**
  - `sinav_gecme_puani` $\rightarrow$ **Sınav Geçme Puanı**
  - `dokuman_kodu` $\rightarrow$ **Doküman Kodu**
  - `dokuman_adi` $\rightarrow$ **Doküman Adı**
  - `revizyon_no` $\rightarrow$ **Revizyon Numarası**
  - `yayin_tarihi` $\rightarrow$ **Yayın Tarihi**
  - `revizyon_tarihi` $\rightarrow$ **Revizyon Tarihi**
  - `dokuman_yolu` $\rightarrow$ **Doküman/Belge Dosya Yolu**
  - `aktif` $\rightarrow$ **Aktiflik Durumu**
...
### 15.3 Onay Kuyruğu Desteği Olmayan Modüllerin Yetki Yönetimi
...
- **Dozimetre Ölçüm Kayıtları ve Takip Aksiyonları** (`dozimetre_service.py`)
- **Fiili Hizmet Süresi ve Dönem Çalışma Hesaplamaları** (`fiili_hizmet_service.py`)

modüllerinde yapılan işlemler onay bekletme kuyruğuna tabi tutulmadan doğrudan veritabanına yazılır.

Yetkilendirme denetimi, standarda uygun olarak RBAC yetki matrisi (`check_service_permission`) üzerinden gerçekleştirilir. Eğer kullanıcının rolüne bu modüller için `yazma`, `guncelleme` veya `silme` yetkileri tanımlanmışsa, rolün `onay_gerektirir` değeri `1` olsa dahi doğrudan işlem gerçekleştirebilir. Yetki tanımlanmamış roller ise standart yetkisiz işlem uyarısıyla bloke edilir.
...
Uygulamanın ana ekranının sağ alt köşesinde (statusbar) yer alan bu alan, kullanıcıya hem canlı durum takibi hem de hızlı bildirim kontrolü sunar:
...
Zil simgesine tıklandığında, durum çubuğunun hemen üzerinde, Windows 11 bildirim kutusuna benzer şekilde çerçevesiz, modern ve yarı saydam gölgelendirmeli bir **Bildirim Çekmecesi** açılır:
...
- **Akıllı Sayfa Yönlendirmesi ve Aksiyonlar (Navigation):** Bir bildirim kartına tıklandığında:
  1. Bildirim otomatik olarak okundu işaretlenir ve bildirim çekmecesi kapatılır.
  2. **Aksiyon Gerektiren Özel Bildirimler (`nobet_devir`):** Eğer bildirim tipi `nobet_devir` ve başlığı `"Nöbet Devir Talebi"` ise, ek bir sayfa açmadan doğrudan onay/red diyalogu açılır. Kullanıcı bu diyalog üzerinden nöbet devrini tek tıkla kabul edebilir veya reddedebilir. Diğer durumlarda (örn. devir tamamlandığında veya onay aşaması değiştiğinde) bildirim doğrudan ilgili nöbet planı listesine yönlendirilir.
  3. **Dozimetre Uyarıları (`dozimetre_uyari`):** Dozimetre sonuç bildirimleri (`dozimetre_uyari`) tıklandığında kullanıcıyı doğrudan dozimetre takip ekranına yönlendirirken, genel sistem bildirimleri (`sistem`) herhangi bir yönlendirme yapmadan sadece bilgilendirme amaçlı çalışır.
  4. **Performans ve Rozet Optimizasyonu:** Uygulamanın durum çubuğundaki okunmamış bildirim adeti (rozet), veritabanı düzeyinde optimize edilmiş hafif sorgularla (`COUNT(*)`) arka planda taranarak sistem kaynaklarına yük getirmeden güncellenmektedir.
...
> **Not:** RADPYS V3, verilerini yerel bir SQLite veritabanı dosyasında saklar. Bu nedenle aşağıdaki yedekleme/geri yükleme işlemleri, bilgisayarınızdaki bu veritabanı dosyası üzerinde çalışır; yedek dosyasını harici bir ortamda (örn. başka bir disk veya bulut depolama) saklamanız veri kaybı risklerine karşı önerilir.
...
Veritabanının performansını korumak amacıyla yapılan bakım işlemlerini (örn. indeks yeniden oluşturma, optimizasyon) kapsar.
...
5. Sistem dosyayı işler; başarılı kayıtlar veritabanına eklenir, hatalı satırlar için bir hata raporu sunulur.
...
- **Toplu Personel İçe Aktarma (Aktif):** Excel şablonundaki yapay ID değerleri yerine **TC Kimlik Numarası** ve **Sicil Numarası** gibi doğal benzersiz anahtarlar üzerinden çakışma kontrolü yapılır. Böylece Excel'deki ID'lerin mevcut veritabanı kayıtlarını ezmesi engellenir. Ayrıca Excel dosyasında yazılı olan ancak sistemde henüz tanımlı olmayan Departman ve Unvan bilgileri, içe aktarım esnasında sistem tarafından otomatik olarak oluşturulur.
...
Toplu içe aktarım (import) işlemleri doğrudan veritabanına büyük miktarda kayıt eklediği veya güncellediği için onay kuyruğuna alınmaya uygun değildir. Veri bütünlüğünü ve güvenliğini korumak amacıyla:

- **Onay Yetkisi Olmayan Roller Engellenmiştir:** Rol tanımlarında `onay_gerektirir = 1` olan roller (Kullanıcı, İzleyici vb.), herhangi bir modülde yazma yetkileri bulunsa dahi toplu içe aktarım işlemini **başlatamazlar**.
- **Yalnızca Yöneticiler İthal Edebilir:** Excel'den toplu içe aktarım butonları ve arka plan servisleri yalnızca onay gerektirmeyen (`onay_gerektirir = 0` / Admin, Yönetici) rollere açık olarak çalışır. Yetkisiz bir kullanıcı bu işlemi tetiklemeye çalıştığında sistem erişimi reddeder.
...
- **Toplu İçe Aktarma Kısıtı:** Excel veya CSV üzerinden yapılan toplu personel ve nöbet ithalatlarında da 15 personel ve 3 nöbet planı limiti tam olarak denetlenmektedir.
- **Güncelleme Limiti (Yama Desteği):** Demo/Deneme sürümünde yeni ticari özellikler içeren ana güncellemeler alınamaz. Ancak programın kararlılığını korumak ve olası kritik hatalardan etkilenmemeniz için kritik hata düzeltme (hotfix) ve yama güncellemeleri demo modunda da otomatik olarak sunulur ve yüklenebilir.
...
### 18.3 Geri Bildirim ve Hata Raporlama Altyapısı
Uygulama çalışırken beklenmedik teknik bir çökme hatasıyla karşılaşırsa:
1. **Çökme Bildirim Ekranı (Crash Dialog):** Program sessizce kapanmaz; karşınıza teknik hata yığın izini (traceback) içeren özel bir hata penceresi gelir.
2. **Hata Kopyalama/E-posta:** Bu penceredeki **"Hata Detayını Kopyala"** butonu ile hatayı panoya alabilir veya **"E-Posta Gönder"** butonu ile varsayılan e-posta istemcinizi (Outlook vb.) açarak hatayı otomatik hazırlanmış şablonla `radpys.iletisim@gmail.com` adresine gönderebilirsiniz.
3. **Destek Log Paketi Oluşturma:** Uygulama içindeyken (çökme olmasa dahi karşılaştığınız sorunlarda) sağ üst köşedeki **Hakkında** butonuna tıklayarak Hakkında ekranını açın. En alttaki **"Destek Paketi (Log) Oluştur"** butonuna basarak, uygulamanın ürettiği tüm KVKK uyumlu log dosyalarını (kişisel veri içermez) masaüstünüze `radpys_destek_log.zip` olarak kaydedebilirsiniz. Bu paketi hata bildirim e-postanıza ekleyerek bize iletmeniz sorunların hızlıca çözülmesini sağlayacaktır.
...
- Yetki veya rolünüz "onay gerektiren" gruptaysa, onay kuyruğu desteği bulunmayan Dozimetre ve Fiili Hizmet modüllerinde doğrudan veri yazabilir veya değiştirebilirsiniz (tabii ki ilgili modüllerde rolünüze ait yazma/güncelleme/silme yetkileri tanımlanmışsa). Bu modüllerde yapılan işlemler onay kuyruğuna alınmadan doğrudan veritabanına işlenir.

- Dinamik alan zenginleştirme işlemleri (`_enrich_optional_fields`) olası FieldProvider hatalarının uygulamayı çökertmesini önlemek için koruma altına alınmıştır.
